...
async function getRedisStatus() {
  const configured = hasRedisConfiguration();
  const redis = getRedis();
  if (!redis) {
    return {
      available: false,
  ...
  try {
    if (typeof redis.ping === 'function') {
      ...
      await redis.set('__acuity_healthcheck__', '1', { ex: 15 });
    }
    return {
      available: true,
      configured: true,
    ...
    redisInitError = err?.message || String(err);
    return {
      available: false,
      configured,
      provider: redisProvider,
...
function getRedis() {
  ...
  const nativeUpstashClient = createNativeUpstashRedisClient();
  if (nativeUpstashClient) {
    redisClient = nativeUpstashClient;
    redisProvider = 'upstash-rest';
    return redisClient;
  }

  const upstashClient = createUpstashRedisClient();
  if (upstashClient) {
    redisClient = upstashClient;
    redisProvider = 'upstash-sdk';
    return redisClient;
  }

  const standardClient = createStandardRedisClient();
  redisClient = standardClient;
  if (standardClient) {
    redisProvider = 'redis-url';
  }
  return redisClient;
}

function createNativeUpstashRedisClient() {
  const creds = getRedisCredentials();
  const url = normalizeEnvString(creds?.url);
  const token = normalizeEnvString(creds?.token);
  ...
  async function runCommand(command) {
    ...
    if (!response.ok) {
      throw new Error(payload?.error || `Upstash REST request failed (${response.status})`);
    }
  ...
  return {
    async lpush(key, value) {
      return runCommand(['LPUSH', key, value]);
    },
    async ltrim(key, start, stop) {
      return runCommand(['LTRIM', key, String(start), String(stop)]);
    },
    async lrange(key, start, stop) {
      return runCommand(['LRANGE', key, String(start), String(stop)]);
    },
    async get(key) {
    ...
    async set(key, value, options = null) {
      if (options && typeof options.ex === 'number') {
        return runCommand(['SET', key, value, 'EX', String(options.ex)]);
      }
      return runCommand(['SET', key, value]);
    },
    async del(key) {
...
function createStandardRedisClient() {
  try {
    const redisUrl = normalizeEnvString(process.env.REDIS_URL);
    ...
    try {
      ...
      if (parsed.protocol === 'redis:' && parsed.hostname && parsed.hostname.endsWith('upstash.io')) {
        clientOptions.socket = { tls: true };
      }
    } catch {
      // Ignore URL parse errors and let the client report them on connect.
    }
...
function hasRedisConfiguration() {
  const explicitUrl = normalizeEnvString(process.env.UPSTASH_REDIS_REST_URL)
    || normalizeEnvString(process.env.KV_REST_API_URL)
    || normalizeEnvString(process.env.REDIS_REST_URL);
  const explicitToken = normalizeEnvString(process.env.UPSTASH_REDIS_REST_TOKEN)
    || normalizeEnvString(process.env.KV_REST_API_TOKEN)
    || normalizeEnvString(process.env.REDIS_REST_TOKEN);
  if (explicitUrl && explicitToken) return true;

  return !!normalizeEnvString(process.env.REDIS_URL);
}
