...
async function findPassKitMemberIdByExternalId(externalId) {
  const cfg = getConfig();
  if (!cfg.PASSKIT_PROGRAM_ID) {
    throw new Error('Missing PASSKIT_PROGRAM_ID for PassKit member lookup');
  }

  async function parseMemberIdFromPayload(data) {
    if (!data) return null;

    if (data.id) return String(data.id);
    if (data.memberId) return String(data.memberId);

    const directMemberIds = Array.isArray(data.memberIds) ? data.memberIds : null;
    if (directMemberIds && directMemberIds.length > 0) return String(directMemberIds[0]);
    ...
    for (const rows of listCandidates) {
      ...
      if (first?.memberId) return String(first.memberId);
    }

    return null;
  }

  async function queryMemberIdByField(filterField, filterValue) {
    const payload = {
      filters: {
        limit: 1,
        offset: 0,
    ...
    return parseMemberIdFromPayload(response?.data);
  }
  ...
  try {
    ...
    const byDirectLookup = await parseMemberIdFromPayload(direct?.data);
  ...
  try {
    const byMemberId = await queryMemberIdByField('memberId', externalId);
  ...
  try {
    const byExternalId = await queryMemberIdByField('externalId', externalId);
...
async function deactivatePassKitMembershipByExternalId(externalId, context = {}) {
  ...
  try {
    const response = await passKitRequest('PUT', '/members/member', {
    ...
    return {
      ...
      passKitId: response.data?.id || response.data || null,
    };
  } catch (statusErr) {
    ...
    const memberId = await findPassKitMemberIdByExternalId(externalId);
    if (!memberId) {
      return { success: true, method: 'member_not_found', passKitId: null };
    }

    const response = await passKitRequest('DELETE', '/members/member', { id: memberId });
    return {
      success: true,
      method: 'delete',
      passKitId: response.data?.id || memberId,
    };
  }
}
