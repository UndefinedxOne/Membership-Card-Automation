...
| `PASSKIT_TIER_ID` | Your tier ID | PassKit Dashboard → Your tier/design |
...
### Step 5 (Optional): Add Vercel KV for Persistent Logs
...
3. Vercel automatically injects `KV_REST_API_URL` and related env vars
4. Install the KV package: add `"@vercel/kv": "^2.0.0"` to your `package.json` dependencies
5. Redeploy
...
- **Activity log** (persistent with Vercel KV, ephemeral without)
...
| `GET`  | `/api/logs` | Activity log (requires KV) |
...
| Logs | In-memory (lost on restart) | Vercel function logs + optional KV |
...
- Without Vercel KV, the dashboard log won't persist between function calls
- Check Vercel Dashboard → Functions → Logs for all output
- Add Vercel KV for persistent in-app logs
